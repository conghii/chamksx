...
export const addEmployee = (data: Partial<Employee>) => callPost<{ id: string; pin_code: string }>('addEmployee', { data });
...
export const checkPinExists = (pin_code: string) => callGet<{ exists: boolean }>('checkPinExists', { pin_code });

// ==================== ATTENDANCE APIs ====================

export const checkIn = (employee_id: string, shift: string) =>
  callPost<CheckInResponse>('checkIn', { employee_id, shift });
...
export const assignTask = (data: Partial<TaskAssignment>) =>
  callPost<{ id: string }>('assignTask', { data });
...
export const addOrderWithTasks = (data: Record<string, unknown>) =>
  callPost<{ id: string; order_code: string; tasks_created: number }>('addOrderWithTasks', { data });
